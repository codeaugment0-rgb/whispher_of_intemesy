...
(function () {
  ...
  async function copyRandomScene(button) {
    ...
    try {
      ...
      // Show success state
      button.innerHTML = `
        <svg class="w-4 h-4 mr-2 text-gray-700" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
        </svg>
  ...
  // Cleanup loading states (simplified without spinners)
  function cleanupLoadingStates() {
    // Remove opacity effects
    document.querySelectorAll('.opacity-75').forEach(element => {
  ...
  // Remove loading states on back/forward navigation
  window.addEventListener('popstate', cleanupLoadingStates);

  // Remove loading states on page show (for additional back/forward handling in some browsers)
